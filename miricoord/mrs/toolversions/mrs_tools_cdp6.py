...
def xytoabl(xin,yin,channel,**kwargs):
    ...
    # Slice map
    d2c_slice=distfile['Slice_Number'].data
...
def abltoxy(alin,bein,lamin,channel,**kwargs):
    ...
    # Read the Slice Map
    c2d_slice=distfile['Slice_Number'].data
...
def abtov2v3(alin,bein,channel):
    ...
    convtable=distfile['albe_to_XANYAN'].data
    # Determine which rows we need
    v2index=(np.where(convtable['Label'] == 'T_CH'+channel+'_XAN'))[0][0]
    v3index=(np.where(convtable['Label'] == 'T_CH'+channel+'_YAN'))[0][0]
    ...
    # Apply transform to XAN,YAN
    xan=conv_v2[1]+conv_v2[2]*alpha+conv_v2[3]*beta+conv_v2[4]*alpha*beta
    yan=conv_v3[1]+conv_v3[2]*alpha+conv_v3[3]*beta+conv_v3[4]*alpha*beta
...
def v2v3toab(v2in,v3in,channel):
    ...
    # Read the distortion table
    convtable=distfile['XANYAN_to_albe'].data
    # Determine which rows we need
    ...
    alpha=conv_al[1]+conv_al[2]*xan+conv_al[3]*yan+conv_al[4]*xan*yan
    beta=conv_be[1]+conv_be[2]*xan+conv_be[3]*yan+conv_be[4]*xan*yan
