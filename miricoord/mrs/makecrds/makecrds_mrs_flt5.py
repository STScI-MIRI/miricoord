...
MIRI MRS using reference files delivered with FLT-5:

MIRI_FM_MIRIFULONG_34LONG_DISTORTION_FLT5.fits
MIRI_FM_MIRIFUSHORT_12MEDIUM_DISTORTION_FLT5.fits
MIRI_FM_MIRIFULONG_34MEDIUM_DISTORTION_FLT5.fits
MIRI_FM_MIRIFUSHORT_12SHORT_DISTORTION_FLT5.fits
...
def create_flt5_all(outdir, **kwargs):
    # Regular grating wheel configurations
    detbands='12A','12B','12C','34A','34B','34C'
    nbands=len(detbands)
    for i in range(nbands):
        create_flt5_setfiles(detbands[i],outdir,**kwargs)
...
def create_flt5_setfiles(detband,outdir, **kwargs):
...
def create_flt5_setxfiles(detxband,outdir,**kwargs):
    rootdir=os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    rootdir=os.path.join(rootdir,'data/fits/flt5/')
    ...
    specfile=outdir+'jwst_miri_mrs'+detxband+'_specwcs_flt5.asdf'
    wavefile=outdir+'jwst_miri_mrs_wavelengthrange_flt5.asdf'
    refs={'distortion': distfile, 'regions':regfile, 'specwcs':specfile, 'wavelengthrange':wavefile}
    print('Working on: '+detxband)
    create_flt5_onexreference(fname1,fname2,refs,**kwargs)
    print('Created: '+detxband)

    
#############################
    
def create_flt5_onereference(fname, ref, **kwargs):
    """
...
def create_flt5_onexreference(fname1, fname2, ref, **kwargs):
    """
...
# Detband is (e.g.) '12A'
def test_flt5_onereference(detband,refs):
